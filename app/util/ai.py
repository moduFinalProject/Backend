...
async def resume_standard_feedback(resume: dict)-> ResumeFeedbackAI:
    ...
    prompt = ChatPromptTemplate([
        ("system","당신은 전직 한 회사의 인사과 팀장이자 이력서 첨삭 전문가 입니다. 이력서 정보를 받고 회사에 취업할 수 있도록 이력서를 피드백 해주세요, 단 사실에 근거해야 합니다."),
        ("user","{resume}")
    ])
...
async def resume_feedback_with_posting(resume: dict, posting: dict)-> ResumeFeedbackAI:
    ...
    prompt = ChatPromptTemplate([
        ("system","당신은 전직 {company} 회사의 인사과 팀장이자 이력서 첨삭 전문가 입니다. 이력서와 공고 정보를 받고 해당 회사에 취업할 수 있도록 이력서를 피드백 해주세요, 단 사실에 근거해야 합니다."),
        ("user","{resume}, {posting}")
    ])
    
    chain = prompt | llm.with_structured_output(ResumeFeedbackAI)
    
    result = chain.invoke({"compant":posting.get("company"),"resume":resume, "posting":posting})
